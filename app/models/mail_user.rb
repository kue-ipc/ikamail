class MailUser < ApplicationRecord
  ...
  has_many :primary_memberships, -> { where(primary: true) }, class_name: "MailMembership", inverse_of: :mail_user
  has_many :secondary_memberships, -> { where(primary: false) }, class_name: "MailMembership", inverse_of: :mail_user
  ...
  has_many :applicable_recipients, -> { where(excluded: false) }, class_name: "Recipient", inverse_of: :mail_user
